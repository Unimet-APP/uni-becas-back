...
class EmailService {
  ...
  /**
   * Enviar campaña masiva de correos
   * @param {Array} destinatarios - Array de objetos con { email, nombre, ...datos personalizados }
   * @param {string} asunto - Asunto del correo
   * @param {string} htmlTemplate - Template HTML con placeholders {{nombre}}, {{variable}}, etc.
   * @param {Object} opciones - Opciones adicionales (remitente, etc.)
   * @returns {Object} Resultado del envío { exitosos, fallidos, detalles }
   */
  async sendCampañaMasiva(destinatarios, asunto, htmlTemplate, opciones = {}) {
    const resultados = {
    ...
    console.log(`📧 Iniciando campaña masiva a ${destinatarios.length} destinatarios`);
    ...
    for (const destinatario of destinatarios) {
      try {
        ...
        Object.keys(destinatario).forEach(key => {
          const regex = new RegExp(`{{${key}}}`, 'g');
          htmlPersonalizado = htmlPersonalizado.replace(regex, destinatario[key] || '');
        });

        const mailOptions = {
          from: opciones.from || process.env.EMAIL_FROM || `Sistema de Becas UNIMET <${process.env.EMAIL_USER}>`,
        ...
        console.error(`❌ Error al enviar email a ${destinatario.email}:`, error.message);
      }
    }

    console.log(`📊 Campaña finalizada: ${resultados.exitosos}/${resultados.total} exitosos`);
    return resultados;
  }

  /**
   * Template genérico para campañas masivas
   * @param {string} titulo - Título de la campaña
   * @param {string} contenido - Contenido principal (puede incluir HTML)
   * @param {string} ctaTexto - Texto del botón CTA (opcional)
